...
export const ReviewStep = memo(function ReviewStep({
	...
	const urgencyBadge = getUrgencyBadge(formData.urgency as string);
			...
			<div className="space-y-6">
				...
				<div className="space-y-4">
					<div className="flex justify-between items-start">
						<h2 className="text-xl font-semibold text-foreground break-words pr-8">
							{formData.title || ""}
						</h2>
						<Button
							variant="ghost"
							...
							className="h-8 w-8 rounded-full p-0 hover:bg-muted flex-shrink-0"
						>
						...
						<div className="p-5 bg-card">
							<div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
								{previewImages.map((image, index) => (
									<div
										key={`img-${index}-${image.preview.substring(0, 20)}`}
